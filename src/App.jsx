...
function useStorage(key, def) {
  const [data, setData] = useState(() => { try { const stored = localStorage.getItem(key); return stored ? JSON.parse(stored) : def } catch (e) { return def } });
  const [loaded] = useState(true);
  const save = useCallback((nd) => { setData(nd); try { localStorage.setItem(key, JSON.stringify(nd)) } catch (e) { console.error(e) } }, [key]);
...
function DashboardPage({ items, expenses, setPage }) {
  ...
  return (
    <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }}>{stats.map((s, i) => (
...
function InventoryPage({ items, saveItems, toast }) {
  ...
  return (
    <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* Table */}
      <div style={{ background: "#171B24", border: "1px solid #1E2330", borderRadius: 14, overflow: "hidden" }}>
...
function ListingsPage({ items, saveItems, toast }) {
  ...
  return (<div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 20 }}>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>{[{ label: "Active Listings", value: listed.length, color: "#6366F1" }, { label: "Total Listed Value", value: fmt(totalValue), color: "#10B981" }, { label: "Potential Profit", value: fmt(potentialProfit), color: "#F59E0B" }].map((s, i) => (<div key={i} style={{ background: "#171B24", border: "1px solid #1E2330", borderRadius: 12, padding: "18px 20px" }}><div style={{ fontSize: 11, color: "#5A6478", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.5px" }}>{s.label}</div><div style={{ fontSize: 28, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", letterSpacing: -1, marginTop: 4, color: s.color }}>{s.value}</div></div>))}</div>
    <div style={{ background: "#171B24", border: "1px solid #1E2330", borderRadius: 14, overflow: "hidden" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><thead><tr style={{ borderBottom: "1px solid #1E2330" }}>{["ID", "Product", "Channel", "Cost", "Price", "Profit", "Margin", "Condition", ""].map(h => (<th key={h} style={{ padding: "12px 14px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#5A6478", textTransform: "uppercase" }}>{h}</th>))}</tr></thead><tbody>
      {listed.length === 0 ? <tr><td colSpan={9} style={{ padding: 40, textAlign: "center", color: "#5A6478" }}>No active listings. Move items to "Listed" status.</td></tr> :
        listed.map(l => {
          const profit = (l.price || 0) - l.cost; const margin = l.price > 0 ? Math.round((profit / l.price) * 100) : 0; return (<tr key={l.id} style={{ borderBottom: "1px solid #1E2330" }} onMouseEnter={e => e.currentTarget.style.background = "#1C2130"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
...
function PurchasingPage({ pos, savePOs, toast }) {
  ...
  return (<div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 20 }}>
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, flex: 1, marginRight: 16 }}>
    ...
    <div style={{ background: "#171B24", border: "1px solid #1E2330", borderRadius: 14, overflow: "hidden" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><thead><tr style={{ borderBottom: "1px solid #1E2330" }}>{["PO #", "Source", "Pallets", "Units", "Cost", "Cost/Unit", "Status", "ETA", "Notes", ""].map(h => (<th key={h} style={{ padding: "12px 12px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#5A6478", textTransform: "uppercase" }}>{h}</th>))}</tr></thead><tbody>
      {pos.map(po => {
        const canAdv = PO_STATUSES.indexOf(po.status) < PO_STATUSES.length - 1; return (<tr key={po.id} style={{ borderBottom: "1px solid #1E2330" }} onMouseEnter={e => e.currentTarget.style.background = "#1C2130"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
    ...
    {tab === "aging" && <>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: "#171B24", border: "1px solid #1E2330", borderRadius: 14, padding: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {agingData.map((a, i) => {
              const cost = totalAgingCost[a.range] || 0; const totalCost = agingItems.reduce((s, it) => s + it.cost, 0); const pct = totalCost > 0 ? (cost / totalCost) * 100 : 0;
      ...
      <div style={{ background: "#171B24", border: "1px solid #1E2330", borderRadius: 14, padding: 24 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><thead><tr style={{ borderBottom: "1px solid #1E2330" }}>{["ID", "Product", "Category", "Status", "Cost", "Price", "Days Held", "Source"].map(h => (<th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#5A6478", textTransform: "uppercase" }}>{h}</th>))}</tr></thead><tbody>
          {[...agingItems].sort((a, b) => new Date(a.received) - new Date(b.received)).slice(0, 10).map(i => {
            const days = Math.floor((now - new Date(i.received)) / (1000 * 60 * 60 * 24)); return (<tr key={i.id} style={{ borderBottom: "1px solid #1E2330" }}>
...
function ExpensesPage({ expenses, saveExpenses, toast }) {
  ...
  return (<div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 20 }}>
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, flex: 1, marginRight: 16 }}>
  ...
  return (<div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 20 }}>
    ...
    <div style={{ background: "#171B24", border: "1px solid #1E2330", borderRadius: 14, overflow: "hidden" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><thead><tr style={{ borderBottom: "1px solid #1E2330" }}>{["ID", "Name", "Email", "Phone", "Orders", "Total Spent", "Notes", ""].map(h => (<th key={h} style={{ padding: "12px 14px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#5A6478", textTransform: "uppercase" }}>{h}</th>))}</tr></thead><tbody>
      {filtered.length === 0 ? <tr><td colSpan={8} style={{ padding: 40, textAlign: "center", color: "#5A6478" }}>No customers found</td></tr> :
        filtered.map(c => {
          const st = custStats[c.id] || { orders: c.totalOrders || 0, spent: c.totalSpent || 0 }; return (<tr key={c.id} style={{ borderBottom: "1px solid #1E2330" }} onMouseEnter={e => e.currentTarget.style.background = "#1C2130"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
    ...
    <div style={{ background: "#171B24", border: "1px solid #1E2330", borderRadius: 14, overflow: "hidden" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><thead><tr style={{ borderBottom: "1px solid #1E2330" }}>{["ID", "Item", "Customer", "Carrier", "Tracking", "Service", "Cost", "Status", "Ship Date", ""].map(h => (<th key={h} style={{ padding: "12px 12px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#5A6478", textTransform: "uppercase" }}>{h}</th>))}</tr></thead><tbody>
        {filtered.length === 0 ? <tr><td colSpan={10} style={{ padding: 40, textAlign: "center", color: "#5A6478" }}>No shipments found</td></tr> :
          filtered.map(sh => {
            const canAdv = SHIPMENT_STATUSES.indexOf(sh.status) < SHIPMENT_STATUSES.length - 1 && sh.status !== "Exception"; return (<tr key={sh.id} style={{ borderBottom: "1px solid #1E2330" }} onMouseEnter={e => e.currentTarget.style.background = "#1C2130"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
    ...
    <div style={{ background: "#171B24", border: "1px solid #1E2330", borderRadius: 14, padding: 24 }}>
      ...
      {monthSold.length === 0 ? <div style={{ textAlign: "center", padding: 32, color: "#5A6478", fontSize: 13 }}>No items sold this month</div> :
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><thead><tr style={{ borderBottom: "1px solid #1E2330" }}>{["ID", "Product", "Channel", "Cost", "Price", "Profit", "Margin", "Sold Date"].map(h => (<th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#5A6478", textTransform: "uppercase" }}>{h}</th>))}</tr></thead><tbody>
          {monthSold.map(i => {
            const profit = (i.price || 0) - i.cost; const margin = i.price > 0 ? Math.round((profit / i.price) * 100) : 0; return (<tr key={i.id} style={{ borderBottom: "1px solid #1E2330" }}>
              <td style={{ padding: "10px 12px", fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: "#818CF8" }}>{i.id}</td>
...
export default function App() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", background: "#0A0C10", color: "#E8ECF4", fontFamily: "'DM Sans',sans-serif" }}>
      <style>{`@keyframes slideUp{from{transform:translateY(20px);opacity:0}to{transform:translateY(0);opacity:1}}input[type=checkbox]{width:16px;height:16px;cursor:pointer}`}</style>
      {toastMsg && <Toast message={toastMsg.message} type={toastMsg.type} onClose={() => setToastMsg(null)} />}

      {/* Sidebar */}
      <div style={{ width: 220, minWidth: 220, background: "#0D0F14", borderRight: "1px solid #1E2330", display: "flex", flexDirection: "column", padding: "20px 10px", gap: 1, overflowY: "auto" }}>
        ...
        {nav.map(item => (
          <button key={item.id} onClick={() => setPage(item.id)} style={{ display: "flex", alignItems: "center", gap: 9, padding: "9px 12px", borderRadius: 8, cursor: "pointer", transition: "all 0.15s", fontSize: 13, border: "none", width: "100%", textAlign: "left", fontFamily: "'DM Sans',sans-serif", background: page === item.id ? "rgba(99,102,241,0.12)" : "transparent", color: page === item.id ? "#818CF8" : "#8B95A9", fontWeight: page === item.id ? 500 : 400 }}>
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "18px 32px", borderBottom: "1px solid #1E2330", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(10,12,16,0.85)", backdropFilter: "blur(16px)", flexShrink: 0 }}>
          <div><h1 style={{ fontSize: 20, fontWeight: 600, letterSpacing: -0.3, margin: 0 }}>{titles[page]?.[0]}</h1><p style={{ fontSize: 13, color: "#8B95A9", margin: "2px 0 0" }}>{titles[page]?.[1]}</p></div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {alertCount > 0 && <button onClick={() => setPage("alerts")} style={{ padding: "6px 12px", borderRadius: 7, border: "1px solid rgba(245,158,11,0.3)", background: "rgba(245,158,11,0.08)", color: "#FBBF24", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", display: "flex", alignItems: "center", gap: 6 }}>{icon(I.warn, 14)}{alertCount} alert{alertCount !== 1 ? "s" : ""}</button>}
          </div>
        </div>
